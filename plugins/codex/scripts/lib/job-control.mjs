...
export function resolveResultJob(cwd, reference) {
  ...
  if (selected) {
    ...
    if (!readStoredJob(workspaceRoot, selected.id)) {
      throw new Error(
        `Job ${selected.id} finished with status "${selected.status}", but its stored result is missing (pruned or quarantined). Run /codex:status ${selected.id} to see what's left.`
      );
    }
