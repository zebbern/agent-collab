...
- Treat `--effort <value>`, `--model <value>`, and `--profile <deep|fast>` as runtime controls and do not include them in the task text you pass through.
- Leave `--profile` unset by default. Only add `--profile <deep|fast>` when the user explicitly asks for that named profile. If the user asked for `deep`, pass `--profile deep`; if they asked for `fast`, pass `--profile fast`.
