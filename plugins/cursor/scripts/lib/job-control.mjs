...
function legacyResidueHint(workspaceRoot) {
  try {
    ...
    if (shards.length > 0) {
      return ` Note: an older plugin version left job state under ${shards[0]}; run /cursor:doctor for details.`;
    }
  } catch {
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run /cursor:status to inspect known jobs.${legacyResidueHint(workspaceRoot)}`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run /cursor:status to inspect active jobs.${legacyResidueHint(workspaceRoot)}`);
  }
